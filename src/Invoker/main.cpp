...
void cppUnquotedServicePaths() {
	...
	if (name.length() > 0) {
		...
		output("");
		if (choice == "1") {
			manageService(name, START);
		}
		else if (choice == "2") {
			manageService(name, STOP);
		}
		else if (choice == "3") {
			manageService(name, RESTART);
		}
		else {
			output("Invalid choice");
		}
		...
		if (isPositiveNumber(choice)) {
			...
			if (ch >= 1 && ch <= 20) {
				...
				switch (ch) {
					...
					case 11: { cppDumpProcMemory();         break; }
